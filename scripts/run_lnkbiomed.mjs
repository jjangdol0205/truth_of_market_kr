...
async function analyzeTickerLocal(tickerInput, numericTicker, reportType = "research", quarter = "") {
    ...
    **OUTPUT JSON STRUCTURE:**
    ...
    for (const modelName of modelsToTry) {
        try {
            console.log(`Trying model: ${modelName} for ${reportType}...`);
            ...
            try {
                ...
                console.log(`Parsed JSON keys:`, Object.keys(analysis)); 
            ...
            console.log(`Saving ${reportType} report locally...`);
            const jsonFilename = `lnkbiomed_${reportType}_q4_2025.json`;
            const mdFilename = `lnkbiomed_${reportType}_q4_2025.md`;
            
            fs.writeFileSync(jsonFilename, JSON.stringify(analysis, null, 2));
            if (reportType === "research") {
                 fs.writeFileSync(mdFilename, finalMarkdown);
            }
            console.log(`Saved locally as ${jsonFilename} and ${mdFilename}`);
            ...
            try {
                ...
                if (error) {
                    ...
                    console.log(`Successfully saved ${reportType} report to Supabase!`);
                }
            } catch (err) {
...
async function main() {
    console.log("Starting L&K Biomed Deep Research & Earning Guidance...");
    const tickerName = "엘앤케이바이오"; 
    const numericTicker = "156100"; 

    // 1. Run Research
    console.log("=== Running Research Report ===");
    await analyzeTickerLocal(tickerName, numericTicker, "research", "");

    // 2. Run Earnings Guidance QQ4 2025
    console.log("=== Running Earning Guidance Report ===");
    await analyzeTickerLocal(tickerName, numericTicker, "earnings", "Q4 2025");
    
    console.log("Finished running reports.");
}
